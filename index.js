...
// ================= Global Variables =================
const ownerNumber = [config.OWNER_NUMBER || "94721746744"];
const botName = "Hasiya MD";
...
const MEGA_URLS = {
  plugins: "https://mega.nz/folder/br4xSLBL#F8LPSfbLcZUQ0Uy5M47NzQ",
  lib:     "https://mega.nz/folder/WnZDhYiJ#emOrQ6Lptp_GKbfMiY2pfg",
...
async function ensureBotFiles() {
  // auth_info_baileys folder ensure
  ensureDirSync(path.join(__dirname, "auth_info_baileys"));
  ...
  // SESSION — load from SESSION_ID env var (hasi& / base64 / MEGA link)
  await loadSession();
}
...
async function loadSession() {
  ...
  if (!sessionId) {
    console.log("[HASIYA MD] No SESSION_ID found. Please set it in .env or platform env vars.");
    return false;
  }
  ...
  // Valid creds already exist — skip
  ...
  // ── Normalize hasi& short IDs to full MEGA links ──
  if (sessionId.startsWith("hasi&") && sessionId.length < 100) {
    sessionId = "https://mega.nz/file/" + sessionId.slice(5);
    console.log("[SESSION] hasi& short ID detected → converting to MEGA link.");
  }
  ...
  try {
    ...
    for (const prefix of ["HASIYA_MD_", "HASIYA_", "hasi&"]) {
  ...
  console.log("[SESSION] Could not load session. Set SESSION_ID to hasi&<id>, a MEGA link, or base64 string.");
  return false;
}
...
// Tracks last denied-reply time per user — stops spam bot replies
const _accessDeniedCooldown = new Map();
const ACCESS_DENIED_COOLDOWN_MS = 60 * 1000; // 1 min cooldown per user
...
async function startBot(sessionId, authPath, envConfig) {
  ...
  // Store conn reference so loadPlugins() can attach listeners after plugins load
  ...
  conn.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      // Update conn reference on every reconnect
      if (!global._activeConns) global._activeConns = new Map();
      global._activeConns.set(sessionId, conn);
      // Try attach now — if plugins not loaded yet, loadPlugins() will attach after load
...
await conn.sendMessage(
  ...
  conn.ev.on("messages.upsert", async (mkk) => {
    try {
      ...
      // ================= ACCESS CONTROL (runs for ALL messages) =================
      // Check access BEFORE anything else — blocks users in private/inbox/group mode
      // IMPORTANT: If user has an active cinesubz/plugin state (awaiting number reply),
      // we allow the message through so the listener can handle it.
      ...
      if (!isOwner && !_hasActiveState && typeof global.checkAccess === "function") {
        ...
        if (!access.allowed) {
          // Only reply to commands, and only once per minute to stop spam
          if (isCmd && shouldSendDenied(sessionId, senderNumber)) {
            await conn.sendMessage(from, { text: access.reason }, { quoted: mek });
          }
          return; // Always stop here — no command runs for blocked users
        }
      }
      // ===========================================================================
      ...
      if (isCmd && commandText === "restart") {
        ...
        await conn.sendMessage(from, { text: "🔄 *Hasiya MD* is restarting...\n\n_Please wait a few seconds._" }, { quoted: mek });
...
app.get("/", (req, res) => res.send(`Hasiya Movie Bot is Running. Active sessions: ${activeSessions.size}`));
...
function loadPlugins() {
  ...
  // Attach plugin listeners (cinesubz etc.) AFTER plugins are loaded
  // This fixes the timing issue where connection open fires before plugins load
  if (typeof global.attachCinesubzListener === 'function') {
